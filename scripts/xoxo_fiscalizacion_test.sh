...
# ---------- FUNCIONES ----------
# Fiscalía XOXO: inspección y validación de contratos
audit_contract() {
    local file="$1"
...
process_file() {
    ...
    audit_contract "$FILE" || { echo "💥 Contrato eliminado por XOXO"; rm "$FILE"; return; }
...
echo "🔄 Iniciando monitor de inbox de Gemini para Hormiga 10 (PRIMER PLANO)..."
...
while true; do
    ...
    if [ "$FILES_FOUND" = false ]; then
        echo "⏳ $(date +'%Y-%m-%d %H:%M:%S') — No hay nuevos contratos, monitor activo..."
    fi
